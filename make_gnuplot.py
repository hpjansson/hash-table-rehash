...
proper_names = {
    'boost_unordered_map': 'Boost 1.66 unordered_map',
    'stl_unordered_map': 'GCC 7.2 std::unordered_map',
    'google_sparse_hash_map': 'Google sparsehash 2.0.3 sparse',
    'google_dense_hash_map': 'Google sparsehash 2.0.3 dense',
    'glib_hash_table': 'Glib 2.56 GHashTable',
...
for line in lines:
    ...
    by_benchtype.setdefault("%s" % benchtype, {}).setdefault(program, []).append([nkeys, nbytes, runtime])

chart_data = {}

for i, (benchtype, programs) in enumerate(by_benchtype.items()):
    outfile = open('results/gnuplot/%s.txt' % benchtype, 'w')

    for j, program in enumerate(program_slugs):
        data = programs[program]

        print >>outfile, '"%s"' % proper_names [program]

        for k, (nkeys, nbytes, runtime) in enumerate(data):
            print >>outfile, '%d %d %f' % (nkeys, nbytes, runtime)

        print >>outfile, '\n'

    outfile.close()

subprocess.call(['gnuplot', 'gnuplot/do-plots.cfg'])
